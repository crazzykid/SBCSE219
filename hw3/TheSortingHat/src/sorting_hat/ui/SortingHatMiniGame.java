...
public class SortingHatMiniGame extends MiniGame
{
	...
	img = loadImage(imgPath + undoButton);
        ...
        sT.addState(SortingHatTileState.VISIBLE_STATE.toString(), img);
      //   String tileStackMouseOver = props.getProperty(SortingHatPropertyType.IMAGE_TILE_BACKGROUND_MOUSE_OVER);
        //sT = new SpriteType(ALGORITHM_TYPE);
    ...
    public void updateGUI()
    {
      ...
      //     props.getPropertyOptionsList(SortingHatPropertyType.values()
                  ArrayList<SortingHatTile> tilesToSort;
                    
            tilesToSort =  ((SortingHatDataModel)data).getTilesToSort();
        ...
        while (titlesIt.hasNext())
        {
            ...
            // ARE WE ENTERING A BUTTON?
            if (titles.getState().equals(SortingHatTileState.VISIBLE_STATE.toString()))
            {
                if (titles.containsPoint(data.getLastMouseX(), data.getLastMouseY()))
                {
                    titles.setState(SortingHatTileState.MOUSE_OVER_STATE.toString());
                     // System.out.println(titles.getState()+" section 1");
                }
            }
            // ARE WE EXITING A BUTTON?
            else if (titles.getState().equals(SortingHatTileState.MOUSE_OVER_STATE.toString()))
            {
                 if (!titles.containsPoint(data.getLastMouseX(), data.getLastMouseY()))
                {
                    titles.setState(SortingHatTileState.VISIBLE_STATE.toString());
